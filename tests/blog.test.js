...
describe('When logged in', async () => {
    ...
    describe('And using valid inputs', async () => {
        beforeEach(async () => {
            await page.type('.title input', 'My Title automated');
        ...
        test('Submitting then saving adds blog to index page', async () => {
            ...
            expect(title).toEqual('My Title automated');
        ...
        const result = await page.evaluate(
            () =>  {
                return fetch('/api/blogs', {
                    ...
                    body: JSON.stringify({ title: 'My Title automated', content: 'My Content automated' })
                }).then(res => res.json());
                // fetch return raw data, so convert it to JSON
            }
        )
